import { EnviarPedidosProps, EnviarPedidosItensProps, PedidoProps, PedidoItensProps, ClienteProps, CondPagProps, CarrinhoProdutoComboProps } from './types'
    ...
    if(true){
      const itensBase: any[] = itens.map((produto: EnviarPedidosItensProps) => {
        return {
          ...
          valorBonificado: 0,
          valorBruto:
            ((Number(produto.preco_item) * produto.qtde_unit) + (((Number(produto.preco_item)) * Number(produto.qtde_prod))) * produto.qtde_cx),
          valorDescontoCapa: 0,
          valorDescontoItem: 0,
      ...
      const pedidoBase: PedidoProps = {
        ...
        cpfCnpj: !!pedido.cpf_cnpj_cli ? pedido.cpf_cnpj_cli.substring(2,99) : "41952551000999",
        ...
        dataEntrega: !!pedido.data_entrega ? pedido.data_entrega.replaceAll('-', '') : null,
        ...
        percTaxaFinanc: 0,
        qtdSatelites: 0,
        qtdeAvulsa: itensBase.flat().reduce( (prev: number, curr: PedidoItensProps) => prev + curr.qtdeAvulsa, 0),
        qtdeCaixa: itensBase.flat().reduce( (prev: number, curr: PedidoItensProps) => prev + curr.qtdeCaixa, 0),
        regiaoCliente: pedido.cod_cli.substring(0, 4),
        rota: "01",
        statusControleERP: "",
        statusPedidoHeishop: "",
        ...
        urlPortal: "",
        valorBonificado: 0,
        valorBruto: itensBase.flat().reduce( (prev: number, curr: PedidoItensProps) => prev + curr.valorLiquidoFinal, 0),
        valorDesconto: 0,
        valorFinal: itensBase.flat().reduce( (prev: number, curr: PedidoItensProps) => prev + curr.valorLiquidoFinal, 0),
        valorImpostoBarreira: 0,
        valorIpi: 0,
        valorLiquido: itensBase.flat().reduce( (prev: number, curr: PedidoItensProps) => prev + curr.valorLiquidoFinal, 0 ),
        valorVerba: 0,
...
export async function prePedido(req: Request, res: Response) {
  ...
  const itens = !!carrinho ? carrinho.map((each: any) => {
    ...
    if (each.produtos !== undefined) {
      return each.produtos.map((produto: CarrinhoProdutoComboProps) => {
        ...
        return {
          id: `${counter}-${cliente.cod_cli}/${cliente.final_atendimento.replace(/[-:.TZ]/g, "").substring(0,12)}`,
          seq_item: 0,
          ...
          valor_total_item:
            (Number(produto.preco_prod) / Number(produto.qtde_prod)) * produto.quantidadeAvulsa 
              +
  ...
  if(momento_atual < '10:30:00' || momento_atual > '20:36:00'){
    ...
    return res.status(201).json({retorno_envio: 'Pedido salvo no banco mas não enviado', pedido});
  }
...
export async function postPedidosOffline(req: Request, res: Response) {
  ...
  const retorno = await Promise.all(pedidos.map(async (pedido) => {
...
export async function transmitirPedidos(req: Request, res: Response) {
  ...
  const promises = await Promise.all(
    pedidos.map( async (pedido) => {
      ...
      else if(pedido.data_entrega == null || getBusinessDatesCount(new Date(pedido.data_entrega), new Date()) <= 1){
